 ...
 */
WITH shared_categories_movies AS (
    SELECT
        f2.title
    FROM
        film f1
    JOIN
        film_category fc1 ON f1.film_id = fc1.film_id
    JOIN
        film_category fc2 ON fc1.category_id = fc2.category_id
    JOIN
        film f2 ON f2.film_id = fc2.film_id
    WHERE
...
american_circus_actors AS (
    ...
    JOIN
        film f2 ON f2.film_id = fa2.film_id
    WHERE
        f1.title = 'AMERICAN CIRCUS'
    GROUP BY
        f2.title
...
SELECT
    title
FROM
    shared_categories_movies
INTERSECT
SELECT
    title
FROM
    american_circus_actors
ORDER BY
    title;
